...
class processor(nn.Module):
    def __init__(self, output_size, memory_M, controller, heads):
        ...
        for head in heads:
            if head.head_type() == 'R':
                temp = nn.Linear(1, memory_M)   # Initializing Read with Linear Layer for learning the Read vector initialization
                self.register_buffer("read_bias_" + str(self.num_read_heads), torch.tanh(temp(torch.ones(1))).unsqueeze(0))
                self.init_r += [temp]
                self.num_read_heads += 1

        assert self.num_read_heads > 0, "Read Heads must be at least 1"
    ...
    def create_new_state(self, batch_size):     # Re-creates the New States
        init_r = [torch.tanh(r(torch.ones(1))).unsqueeze(0).clone().repeat(batch_size, 1) for r in self.init_r] # Predicting the initialization
        controller_state = self.controller.create_hidden_state(batch_size)
        heads_state = [head.create_new_state(batch_size) for head in self.heads]    # Weights for the Memory
