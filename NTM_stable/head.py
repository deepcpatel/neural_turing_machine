...
class NTM_read_head(nn.Module):     # Read Head for Memory
    ...
    def __init__(self, memory, controller_size):
        ...
        # Defining linear layer for self learning the weight initialization
        self.weight_init = nn.Linear(1, self.N)

        # Initializing the linear Parameters for weight initializer
        # nn.init.xavier_uniform_(self.weight_init.weight, gain=1.4)
        # nn.init.normal_(self.weight_init.bias, std=0.01)

    def head_type(self):
        return "R"      # Head Type : (R)ead

    def create_new_state(self, batch_size): # The State holds the Weights
        out = F.softmax(self.weight_init(torch.ones(1)), dim=0).unsqueeze(0)   # Initializing weights learned through Backpropogation
        return out.clone().repeat(batch_size, 1)
...
class NTM_write_head(nn.Module):    # Write Head for Memory
    ...
    def fine_tune_params(self, k, beta, g, s, gamma, e, a):   # Fine tunes the params according to the criteria given in the paper
        k = torch.tanh(k)
        beta = F.softplus(beta)
        g = torch.sigmoid(g)
        s = F.softmax(s, dim=1)
    ...
    def forward(self, rnn_embeddings, W_old):
        ...
        k, beta, g, s, gamma, e, a = self.fine_tune_params(k, beta, g, s, gamma, e, a)
        W = self.memory.access_memory(k, beta, g, s, gamma, W_old)
